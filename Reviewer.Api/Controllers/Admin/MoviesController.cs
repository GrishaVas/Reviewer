using Microsoft.AspNetCore.Mvc;
using Reviewer.Infrastructure.Models;
using Reviewer.Services.Abstractions.Admin;
using Reviewer.Services.Models;
using Reviewer.Services.Models.Admin;

namespace Reviewer.Api.Controllers.Admin
{
    public class MoviesController : BaseAdminController<CreateMovieRequest, UpdateMovieRequest, Movie, MovieResponse, IAdminMoviesService>
    {
        public MoviesController(IAdminMoviesService service) : base(service)
        {
        }

        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(MovieResponse))]
        public override Task<IActionResult> Create(CreateMovieRequest create)
        {
            return base.Create(create);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(MovieResponse))]
        public override Task<IActionResult> Get(Guid id)
        {
            return base.Get(id);
        }
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<MovieResponse>))]
        public override Task<IActionResult> GetList(RequestFilters filters)
        {
            return base.GetList(filters);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(MovieResponse))]
        public override Task<IActionResult> Update(UpdateMovieRequest update)
        {
            return base.Update(update);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Guid))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [HttpGet("{name}/id")]
        public IActionResult GetIdByName(string name)
        {
            return Ok(Guid.Empty);
        }
    }
}
